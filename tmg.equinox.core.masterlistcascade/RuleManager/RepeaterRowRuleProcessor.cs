...
namespace tmg.equinox.ruleengine
{
    public class RepeaterRowRuleProcessor : RuleProcessor
    {
        ...
        public RepeaterRowRuleProcessor(RuleDesign rule, string containerName, JObject rowData, JObject formData, FormInstanceDataManager formDataInstance, int formInstanceId, FormDesignVersionDetail detail, string sectionName)
        {
            this._rule = rule;
        ...
        public Dictionary<int, bool> ProcessRulesForChildRows(int? childRowID)
        {
            ...
            if (data != null)
            {
                ...
                for (var idx = 0; idx < data.Count(); idx++)
                {
                    ...
                    bool result = this.ProcessNode(_rule.RootExpression, parentRow);
        ...
        {
            return this.ProcessNode(rule.RootExpression, _rowData);
        }
