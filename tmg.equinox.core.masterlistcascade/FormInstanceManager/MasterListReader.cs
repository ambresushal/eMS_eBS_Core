...
namespace tmg.equinox.expressionbuilder
{
    public class MasterListReader
    {
        ...
        public JObject GetData(int targetFolderVersionID, List<string> sectionNames, ruleprocessor.FormInstanceDataManager formDataInstanceManager, IFolderVersionServices folderVersionServices, IFormDesignService formDesignServices, string ruleAlias, FormDesignVersionDetail sourceDesign)
        {
            JObject completeML = new JObject();
            DateTime folderEffectiveDate = _masterListService.GetEffectiveDate(targetFolderVersionID);
            List<int> formInstanceIds = _masterListService.GetFormInstanceIds(folderEffectiveDate, ruleAlias,"");
            ...
            foreach (int formInstanceId in formInstanceIds)
            {
                ...
                foreach (string secName in sectionNames)
                {
                    ...
                    if (sourceDesign.IsMasterList == designDetails.IsMasterList)
                    {
