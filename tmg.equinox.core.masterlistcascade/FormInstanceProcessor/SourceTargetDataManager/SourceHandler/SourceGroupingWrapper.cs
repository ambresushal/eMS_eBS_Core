...
namespace tmg.equinox.expressionbuilder
{
    public class SourceGroupingWrapper
    {
        ...
        Dictionary<string, int> _ruleAliseformInstances;

        public SourceGroupingWrapper(Dictionary<string, string> sources, Dictionary<string, int> ruleAliseformInstances)
        {
            _sources = sources;
            _ruleAliseformInstances = ruleAliseformInstances;
        }
        ...
        public List<SourceHandlerInput> GetSourceHandlerInput()
        {
            ...
            foreach (var ruleAliasGroup in ruleAliasGroups)
            {
                ...
                sourceHandlerInput.FormInstanceId = _ruleAliseformInstances[ruleAliasGroup.Key];
        ...
        private IEnumerable<IGrouping<string, SourceElement>> GetGroupedRuleAliases()
        {
            IEnumerable<IGrouping<string, SourceElement>> groupedRuleAliases = (from source in _sources
                                                                               select new SourceElement
                                                                               {
                                                                                   ...
                                                                                   Section = source.Value.Substring(source.Value.IndexOf('[') + 1, (source.Value.IndexOf('.') - source.Value.IndexOf('[')) - 1),
