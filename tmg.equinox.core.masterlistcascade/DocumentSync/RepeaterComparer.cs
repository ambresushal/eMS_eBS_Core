...
namespace tmg.equinox.mlcascade.documentcomparer
{
    public class RepeaterComparer
    {
        ...
        public RepeaterCompareResult Compare() 
        {
            ...
            if ((sourceRepeater != null) && (targetRepeater != null))
            {
                List<JToken> sourceItems = ((JArray)sourceRepeater).ToList();
                List<JToken> targetItems = ((JArray)targetRepeater).ToList();


                if (sourceItems.Count > 0 || targetItems.Count > 0)
                {
                    ...
                    else 
                {
                    ...
                    List<JToken> sourceMatches = RepeaterMatchUtil.GetMatchesInRepeater(sourceRepeater, criteria.PrimaryKeysMacro, "Source");

                    //get target primary rows match
                    List<JToken> targetMatches = RepeaterMatchUtil.GetMatchesInRepeater(targetRepeater, criteria.PrimaryKeysMacro, "Target");
        ...
        private RepeaterCompareCriteria GetCompareCriteria(JToken repeaterFirstRow)
        {
            ...
            //get primary fields
            List<string> primaryFields = GetPrimaryFields(repeaterFirstRow);

            //get child fields
            List<string> childFields = GetChildFields(repeaterFirstRow);
            int typeOfMatch = GetTypeOfMatch(primaryFields, childFields);
