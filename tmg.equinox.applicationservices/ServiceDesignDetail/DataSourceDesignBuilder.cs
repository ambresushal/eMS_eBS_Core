...
namespace tmg.equinox.applicationservices.ServiceDesignDetail
{
    internal class DataSourceDesignBuilder
    {
        ...
        internal DataSourceDesignBuilder(int tenantId, int formDesignId, int formDesignVersionId, List<UIElement> formElementList, IUnitOfWorkAsync unitOfWork)
        {
                ...
                foreach (var dsGroup in designGroups)
                {
                    ...
                    foreach (var srcGroup in srcGroups)
                    {
                        ...
                        if (sr != null && sr.Count() > 0)
                        {
                            ...
                            var sourceParentElemntID = items[0].FormDesignID == formDesignId ? GetSourceParentUIElementID(items[0].DataSourceName, items[0].SourceParentUIElementID) :
