...
namespace tmg.equinox.applicationservices
{
    public class JsonParser
    {
        ...
        public bool HasValue(string formInstanceData, string fullPath, string searchValue)
        {
            bool hasValue = false;
                    ...
                    if (values is ExpandoObject)
                    {
                        if (i == count)
                        {
                            if (values.ContainsKey(keyname))
                            {
                                //removed .ToString() method to return the collection 
                                //collection needs to be used in SBC Report.
                                string value = values.FirstOrDefault(x => x.Key == keyname).Value.ToString();
                                hasValue = value == searchValue;
                                return hasValue;
                            }
                        }
                        else
