...
namespace tmg.equinox.integration.qhplite.Ver2016.DocumentExporter
{
    ...
    /// </summary>
    public class DocumentExporter2016BenefitPackageTemplate : IDocumentExporter
    {
        ...
        #region Constructor
        public DocumentExporter2016BenefitPackageTemplate(string folderPath, IList<QhpDocumentViewModel> qhpDocumentList)
        {
        ...
        {
            QHPModelToExcelMapper2016BenefitPackageTemplate mapper = new QHPModelToExcelMapper2016BenefitPackageTemplate(GetPlanBenefitPackages(), _helperfilePath, _filePath);
            ...
            using (ZipArchive zipFile = ZipFile.Open(fullFilePath, ZipArchiveMode.Update))
            {
                var entries = zipFile.Entries;
                var wbEntry = from ent in zipFile.Entries where ent.FullName == "xl/workbook.xml" select ent;
                if (wbEntry != null && wbEntry.Count() > 0)
                {
                    wbEntry.First().Delete();
                    zipFile.CreateEntryFromFile(workBookFile, "xl/workbook.xml");
                }
                RemoveCDataFromSheet(zipFile, "xl/calcChain.xml");
                RemoveCDataFromSheet(zipFile, "xl/worksheets/sheet1.xml");
                //RemoveCDataFromSheet(zipFile, "xl/worksheets/sheet2.xml");
                RemoveCDataFromSheet(zipFile, "xl/worksheets/sheet3.xml");
                //RemoveCDataFromSheet(zipFile, "xl/worksheets/sheet4.xml");
                RemoveCDataFromSheet(zipFile, "xl/worksheets/sheet5.xml");
                RemoveCDataFromSheet(zipFile, "xl/worksheets/sheet6.xml");
                RemoveCDataFromSheet(zipFile, "xl/worksheets/sheet7.xml");
                RemoveCDataFromSheet(zipFile, "xl/worksheets/sheet8.xml");
            }
