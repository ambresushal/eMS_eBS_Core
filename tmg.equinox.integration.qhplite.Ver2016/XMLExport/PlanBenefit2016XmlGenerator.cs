...
{
    public class PlanBenefit2016XmlGenerator : IPlanBenefitXmlGenerator
    {
        ...
        private CostShareVarianceVO GetCostShareVariance(PlanCostSharingAttributes costShareVariance)
        {
            ...
            //generate AV Calculator
            costShare.avCal = GenerateAVCalculatorBenefitDesign(costShareVariance.AVCalculatorAdditionalBenefitDesign);
        ...
        private SBCVO GenerateSBCScenario(SBCScenario sbc)
        {
            ...
            if (sbc != null)
            {
                ...
                sbcScenario.treatmentCoInsurance = GetCellValueExcelObject(sbc.TreatmentOfSimpleFracture.Coinsurance);
                sbcScenario.treatmentCopay = GetCellValueExcelObject(sbc.TreatmentOfSimpleFracture.Copayment);
                sbcScenario.treatmentDeductible = GetCellValueExcelObject(sbc.TreatmentOfSimpleFracture.Deductible);
                sbcScenario.treatmentLimit = GetCellValueExcelObject(sbc.TreatmentOfSimpleFracture.Limit);
            }
            return sbcScenario;
        }

        private AVCalVO GenerateAVCalculatorBenefitDesign(AVCalculatorAdditionalBenefitDesign calDesign)
        {
            AVCalVO avCal = new AVCalVO();
            if (avCal != null)
            {
                avCal.beginPrimaryCareCostSharingAfterSetNumberVisits = GetCellValueExcelObject(calDesign.BeginPrimaryCostSharingAfterSetNumberOfVisits);
                avCal.beginPrimaryCareDeductibleOrCoinsuranceAfterSetNumberCopays = GetCellValueExcelObject(calDesign.BeginPrimaryCostSharingAfterSetNumberOfVisits);
                avCal.maximumCoinsuranceForSpecialtyDrugs = GetCellValueExcelObject(calDesign.MaximumCoinsuranceForSpecialityDrugs);
                avCal.maxNumDaysForChargingInpatientCopay = GetCellValueExcelObject(calDesign.MaximumNumberOfDaysForChargingInpatientCopay);
            }
            return avCal;
        }
