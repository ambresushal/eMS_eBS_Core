...
namespace tmg.equinox.integration.qhplite.Ver2016
{
    ...
    /// </summary>
    public class QHPModelToExcelMapper2020BenefitPackageTemplate
    {
        ...
        #region Constructor
        public QHPModelToExcelMapper2020BenefitPackageTemplate(IList<PlanBenefitPackage> benefitPackageList, string folderPath, string saveFoldePath)
        {
            ...
            FileStream file = new FileStream(emptyTemplatePath, FileMode.Open, FileAccess.Read);
        ...
        private void SetStringValueToExcelCell(QhpToExcelMap map, int rowIndex, string column, string value)
        {
            ...
            if (row != null)
            {
                ...
                if (cell == null)
                {
                    ...
                    //cell = (XSSFCell)row.CreateCell(columnIndex);//.GetCell(columnIndex, MissingCellPolicy.CREATE_NULL_AS_BLANK) as XSSFCell;
                }
                ...
                if (map.QhpSheetType == QHPSheetType.BenefitPackage)
                {
                    switch (map.CellFormat.CellAlignment)
                    {
                        case "Right":
                    ...
                    case CellType.Blank:
                        ...
                        else
                        {
                            if (excelMap != null)
                            {
                                ...
                                else if (excelMap.CellFormat.DataType == QhpCellDataTypes.PERCENTAGETWOPLACED && value != "")
                                {
                                    value = value.Replace("%", "");
                                    double val;
                                    if (double.TryParse(value, out val))
                                    {
                                        SetValueAndFormat(_workbook, cell, val / 100, _format.GetFormat("0.00%"));
                                    }
                                }
                                else if (excelMap.CellFormat.DataType == QhpCellDataTypes.PERCENTAGE && value != "")
                                {
                                    value = value.Replace("%", "");
                                    double val;
                                    if (double.TryParse(value, out val))
                                    {
                                        SetValueAndFormat(_workbook, cell, val / 100, _format.GetFormat("0%"));
                                    }
                                }
                                else if (excelMap.CellFormat.DataType == QhpCellDataTypes.DOLLAR && value != "")
                                {
                                    cell.SetCellType(CellType.Numeric);
                                    value = value.Replace("$", "");
                                    ...
                                    bool decValid = double.TryParse(value, System.Globalization.NumberStyles.Number, culInfo.NumberFormat, out val);
                                    if (decValid)
                                    {
                                        cell.SetCellValue(val);
                                    }
                                }
                                else if (excelMap.CellFormat.DataType == QhpCellDataTypes.LINK && value != "")
                                {
                                    if (Uri.IsWellFormedUriString(value, UriKind.Absolute))
                                    {
                                        ...
                                        NPOI.XSSF.UserModel.XSSFHyperlink link = new XSSFHyperlink(HyperlinkType.Url);
                                        link.Address = (value);
                                        cell.Hyperlink = link;
