using System;
using System.Linq.Expressions;
using System.Web;
using System.Web.Caching;
using tmg.equinox.caching.Interfaces;

namespace tmg.equinox.caching.Adapters
{
    internal class HttpCacheAdapter : ICacheProvider
    {
        #region Private Memebers
       
        private Cache cache;
        private DateTime absoluteExpirationDateTime;
        private TimeSpan slidingExpirationTimeSpan;

        #endregion Private Members

        #region Public Properties
        #endregion Public Properties

        #region Constructor
        public HttpCacheAdapter(DateTime absoluteExpiration,
                                TimeSpan slidingExpiration)
        {
            cache = HttpContext.Current.Cache;
            absoluteExpirationDateTime = absoluteExpiration;
            slidingExpirationTimeSpan = slidingExpiration;
        }

        public HttpCacheAdapter(Cache httpCache,
                                DateTime absoluteExpiration,
                                TimeSpan slidingExpiration)
        {
            cache = httpCache;
            absoluteExpirationDateTime = absoluteExpiration;
            slidingExpirationTimeSpan = slidingExpiration;
        }
        #endregion Constructor
        
        #region Public Methods
      
        public bool Exists(string cacheKey) {

            return cache[cacheKey] != null;
        }

        public T Get<T>(string cacheKey) {

            return Exists(cacheKey) ? (T)cache[cacheKey] : default(T);
        }

        public T Add<T>(string cacheKey, T objectToCache) {
            cache.Insert(cacheKey,
                            objectToCache,
                            null,
                            absoluteExpirationDateTime,
                            slidingExpirationTimeSpan);

            return objectToCache;
        }

        public void Remove(string cacheKey) {

            if (Exists(cacheKey))
            {
                cache.Remove(cacheKey);
            }
        }

        public bool TryGet<T>(string cacheKey, out T cachedObject) {
            bool result = Exists(cacheKey);
            cachedObject = result ? (T)cache[cacheKey] : default(T);

            return result;
        }

        public bool TryAdd<T>(string cacheKey, Expression<Func<T>> cacheItemProvider, out T cachedObject) {
            bool result = Exists(cacheKey);
            cachedObject = result ? (T)cache[cacheKey] : Add<T>(cacheKey, cacheItemProvider.Compile()());

            return result;
        }

        #endregion Public Methods

        #region Private Methods
        #endregion Private Methods

    }
}
