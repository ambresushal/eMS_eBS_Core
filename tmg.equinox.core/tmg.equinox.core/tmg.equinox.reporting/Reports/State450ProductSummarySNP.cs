...
{
    public class State450ProductSummarySNP<QueueItem> : BaseExcelReportExecuter<QueueItem>
    {
        ...
        public void AddStateSheet(ICollection<DataHolder> dt)
        {
            ...
            foreach (var items in dt)
            {
                if (i % 2 != 0 && dt.Count() != i)
                {
                    string sheetName = Convert.ToString(items.Data.ToList()[5].Values.ToList()[0]) + " SNP";
