var folderVersionBaselineML = function () {
    ...
    var URLs = {
        ...
        folderVersionIndex: '/FolderVersion/IndexML?tenantId=1&folderVersionId={folderVersionId}&folderId={folderId}&mode=true'
    };

    //Grid Ids for major and minor version
    var elementIDs = {
        //for dialog for BaseLine
        baseLineButton: '#btnBaselineML',
        baseLineDialogue: "#baseLineDialogueML",
        doneButtonId: "#updateBaseLineML",
        majorVersionGrid: "majorFolderVersionML",
        majorVersionGridJQ: "#majorFolderVersionML",
        minorVersionGrid: "minorFolderVersionML",
        minorVersionGridJQ: "#minorFolderVersionML",
        updatestatus: "#updatestatusML"
    ...
    function addSuccess(xhr) {
        if (xhr.Result === ServiceResult.SUCCESS) {
            ...
            var url = URLs.folderVersionIndex.replace(/\{folderVersionId\}/g, parseInt(xhr.Items[0].Messages[0])).replace(/\{folderId\}/g, folderData.folderId);
    ...
    function loadMajorVersionGrid(isBaseLine, ownerName) {
        ...
        $(elementIDs.majorVersionGridJQ).jqGrid('navGrid', pagerElement, { edit: false, add: false, del: false, search: false });
        //remove scroll bar
    ...
    function loadMinorVersionGrid(isBaseLine, ownerName) {
        ...
        //remove default buttons
        $(elementIDs.minorVersionGridJQ).jqGrid('navGrid', pagerElement, { edit: false, add: false, del: false, search: false });
        //remove scroll bar
    ...
    function init() {
        ...
        $(elementIDs.baseLineButton).on('click', function () {
            folderVersionBaselineML.show(true);
        });
        ...
        $(elementIDs.doneButtonId).on('click', function () {
            ...
            if (folderVersionBaselineML.isRelease == undefined || folderVersionBaselineML.isRelease == false) {
    ...
    return {
        show: function (isBaseline,ownerName) {
            ...
            if (isBaseline) {
                folderVersionBaselineML.setRelease(false);
            } else {
                folderVersionBaselineML.setRelease(true);
            }
