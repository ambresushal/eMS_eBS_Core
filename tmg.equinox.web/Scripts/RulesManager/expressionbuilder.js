function expressionBuilder(ruleObj, containerElementId, searchGridData, logicalOperators, operators) {
...
var repeaterKeyFilter = function (uiElement, searchGridData, target, data, operandName) {
    if (operandName && target == 'Left-Operand') {
        var elementName = operandName.val();
        var objElement = searchGridData.filter(function (obj) {
            return obj.UIElementName == elementName;
        });
        this.uiElement = objElement[0];
    }
    if (target == 'Right-Operand' && data.rightOperandElementName != '') {
        var objElement = searchGridData.filter(function (obj) {
            return obj.UIElementName == data.rightOperandElementName;
        });
        this.uiElement = objElement[0];
    }
    this.searchGridData = searchGridData;
...
repeaterKeyFilter.prototype.show = function () {
    ...
    if (!currentInstance.uiElement) {
        $(repeaterKeyFilter.elementIDs.rptKeysGridJQ).jqGrid('GridUnload');
        return false;
    }
    var parentElementID = currentInstance.uiElement.ParentUIElementId == undefined ? currentInstance.uiElement.Parent : currentInstance.uiElement.ParentUIElementId;
    if (currentInstance.target == 'Left-Operand') {
        if (currentInstance.data != null && currentInstance.data.ltKeyFilter != null && currentInstance.data.ltKeyFilter.length > 0) {
            ...
            getKeyList = repeaterKeyFilter.URLs.leftKeyList.replace(/\{uiElementId\}/g, parentElementID)
    ...
    if (currentInstance.target == 'Right-Operand') {
        if (currentInstance.data != null && currentInstance.data.rtKeyFilter != null && currentInstance.data.rtKeyFilter.length > 0) {
            ...
            getKeyList = repeaterKeyFilter.URLs.rightKeyList.replace(/\{uiElementId\}/g, parentElementID)
                .replace(/\{expressionId\}/g, currentInstance.data.id);
        }
    }
    ...
    if (currentInstance.target == 'Target') {
        if (currentInstance.data != null && currentInstance.data.TargetKeyFilter != null && currentInstance.data.TargetKeyFilter.length > 0) {
            ...
            getKeyList = repeaterKeyFilter.URLs.targetKeyList.replace(/\{uiElementId\}/g, parentElementID)
