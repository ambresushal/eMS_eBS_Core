var tableDeails = function () {
        ...
        $(document).ready(function () {
            for (var i = 0; i < tableInfo.length; i++) {
                if (i != 0) {
                    ...
                    document.getElementById(elementIDs.tableDetails).appendChild(div);
                }
                loadTableDetails(tableInfo[i]);
            }
    ...
    function loadTableDetails(data) {
        ...
        for (var i = 0; i < colmInfo.length; i++) {
            var obj = { headerName: colmInfo[i]["COLUMN_NAME"], field: colmInfo[i]["COLUMN_NAME"], headerTooltip: colmInfo[i]["COLUMN_NAME"], filterParams: { selectAllOnMiniFilter: true, clearButton: true, applyButton: true } };
            columnDefs.push(obj);
        }
