...
(function (root, factory) {
    ...
    var QueryBuilder = function ($el, options) {
        ...
        this.initiator = this.settings.initiator;
        this.initrules = this.settings.rules;
        /**
         * Translations object
         * @member {object}
         * @readonly
    ...
    QueryBuilder.selectors = {
        ...
        rule_searchfilter: '.rule-filter-container [name$=_searchfilter]',
        rule_keycriteria: '.rule-filter-container [name$=_keycriteria]',
        rule_operator: '.rule-operator-container [name$=_operator]',
        rule_value: '.rule-value-container [name*=_value_]',
        rule_enablesearchvalue: '.rule-value-container [name*=_enablesearchvalue]',
        rule_searchvalue: '.rule-value-container [name*=_searchvalue]',
        rule_rgkeycriteria: '.rule-value-container [name*=_rgkeycriteria]',
    ...
    QueryBuilder.DEFAULTS = {
        ...
        inputs_separator: ' , ',
        select_placeholder: 'Please search for element',
    ...
    QueryBuilder.prototype.bindEvents = function () {
        ...
        this.$el.on('change.queryBuilder', Selectors.rule_filter, function () {
            ...
            $(this).prop('title', self.getFilterById($(this).val()).label);
        });
        ...
        (function add(data, group) {
            ...
            data.rules.forEach(function (item) {
                ...
                if (item.rules !== undefined) {
                    ...
                    if (!item.empty) {
                        var filter = self.getFilterById(item.id, !options.allow_invalid);
                        filter.currentItem = item;
                        model.filter = filter;
                        model.$el.find(Selectors.rule_filter).prop('title', filter.label);
                    }
                    ...
                    if (model.operator && model.operator.nb_inputs !== 0) {
                        if (item.value !== undefined) {
                            ...
                            model.$el.find(Selectors.rule_value).prop('title', item.value);
                        }
    ...
    QueryBuilder.prototype._validateValue = function (rule, value) {
        ...
        if (operator.type == "ebs_equals" || operator.type == "ebs_ne") {
...
<li id="{{= it.rule_id }}" class="rule-container new-container-for-rule"> \
...
{{ var optgroup = null; }} \
<select disabled class="form-control query-select" name="{{= it.rule.id }}_filter"> \
  ...
  {{~}} \
  {{? optgroup !== null }}</optgroup>{{?}} \
</select><span class="glyphicon glyphicon-search buttoncursor" name="{{= it.rule.id }}_searchfilter" title="Click to select operand" style="padding:5px"></span>\
<span class="buttoncursor" name="{{= it.rule.id }}_keycriteria" title="Click to enter key critera" style="padding:1px"><i class="material-icons">vpn_key</i></span>';
...
{{?}} \
{{ var optgroup = null; }} \
<select class="form-control query-select {{? it.operators.length === 1 }}hide{{?}}" name="{{= it.rule.id }}_operator"> \
...
<div' + style + ' class="' + filter.input + ' ' + filter.input + '-' + color + '"> \
  ...
  <label for="' + id + '">' + val + '</label> \
    ...
    /**
     * @class eBSElementPicker
     * @memberof module:plugins
     * @description to Select eBS Controls
     * @param {object} [options]
     * @param {string} [options.container='body']
     * @param {int|string} [options.width='auto']
     * @param {boolean} [options.showIcon=false]
     * @throws MissingLibraryError
     */
    QueryBuilder.define('ebs-elementpicker', function (options) {

        var Selectors = QueryBuilder.selectors;

        // init selectpicker
        this.on('afterCreateRuleFilters', function (e, rule) {
            var rls = rule.$el.find(Selectors.rule_filter);
            var rlsSearch = rule.$el.find(Selectors.rule_searchfilter);
            var rlskeyfilter = rule.$el.find(Selectors.rule_keycriteria);
            var initiator = e.builder.initiator;
            $(rlsSearch[0]).on("click", function () {
                var searchGrid = new expressionBuilderSearchDialog(initiator.searchGridData, $(rls[0]), null, false, rule.data);
                searchGrid.show();
            });

            $(rlskeyfilter[0]).on("click", function () {
                var filtergrid = new repeaterKeyFilter(initiator.uiElement, initiator.searchGridData, 'Left-Operand', rule.data, $(rls[0]));
                filtergrid.show();
            });
        });

        this.on('afterAddGroup', function (e, rule) {
            var initiator = e.builder.initiator;
            if (rule.data == null) {
                rule.data = { id: initiator.newId, isRightOperandElement: false, rightOperandElementName: '', ltKeyFilter: null, rtKeyFilter: null };
            }
            initiator.newId = initiator.newId - 1;
        });
        ...
        this.on('afterCreateRuleInput', function (e, rule) {
            var rls = rule.$el.find(Selectors.rule_value);
            rls = rls[rls.length - 1];
            $(rls).after("<input type='checkbox' name='" + rls.name + "_enablesearchvalue' title='Check if want to select operand' style='margin-left:5px'/>");
            $(rls).addClass('query-select');
            var rlsEnableSearch = rule.$el.find(Selectors.rule_enablesearchvalue);
            rlsEnableSearch = rlsEnableSearch[0];
            $(rlsEnableSearch).after("<span class='glyphicon glyphicon-search buttoncursor' name='" + rls.name + "_searchvalue' title='Click to select operand' style='padding:5px;display:none'></span>");
            var rlsSearch = rule.$el.find(Selectors.rule_searchvalue);
            rlsSearch = rlsSearch[0];
            $(rlsSearch).after("<span class='buttoncursor' name='" + rls.name + "_rgkeycriteria' title='Click to enter key criteria' style='padding:1px;display:none;'><i class='material-icons'>vpn_key</i></span>");
            var rlsKeyFilter = rule.$el.find(Selectors.rule_rgkeycriteria);
            rlsKeyFilter = rlsKeyFilter[0];
            if (rule.data == undefined) {
                rule.data = { id: -1, isRightOperandElement: false, ltKeyFilter: null, rtKeyFilter: null };
            }
            if (rule.data.isRightOperandElement == true) {
                $(rlsEnableSearch).prop('checked', true);
                $(rlsSearch).show();
                $(rlsKeyFilter).show();
                $(rls).prop('disabled', true);
            }

            $(rlsEnableSearch).on('change', function () {
                if (this.checked) {
                    rule.data.isRightOperandElement = true;
                    $(rlsSearch).show();
                    $(rlsKeyFilter).show();
                    $(rls).prop('disabled', true);
                    $(rls).val('');
                    ...
                    rule.data.isRightOperandElement = false;
                    $(rlsSearch).hide();
                    $(rlsKeyFilter).hide();
                    $(rls).prop('disabled', false);
                    $(rls).val('');
                    $(rls).prop('title', '');
                }
            });
            var initiator = e.builder.initiator;
            $(rlsSearch).on("click", function () {
                var searchGrid = new expressionBuilderSearchDialog(initiator.searchGridData, $(rls), null, true, rule.data);
                searchGrid.show();
            });
            $(rlsKeyFilter).on("click", function () {
                var filterGrid = new repeaterKeyFilter(initiator.uiElement, initiator.searchGridData, 'Right-Operand', rule.data);
                filterGrid.show();
            });
        });
 ...
 * @throws MissingLibraryError
 */
    QueryBuilder.define('ebs-aliaspicker', function (options) {
        ...
        this.on('afterCreateRuleOperators', function (e, rule) {
            var rls = rule.$el.find(Selectors.rule_operator);
            $(rls).prev().remove();
        });
        ...
        this.on('afterUpdateRuleOperator', function (e, rule) {
            ...
            rls = rls[rls.length - 1];
            $(rls).addClass('query-select');
            if (rule.operator.type == "ebs_any" || rule.operator.type == "ebs_all" || rule.operator.type == "ebs_notany") {
                $(rls).attr('multiple', 'multiple');
                rule.filter.multiple = true;
                if (Array.isArray(rule.filter.currentItem.value)) {
                    $.each(rule.filter.currentItem.value, function (i, e) {
                        $(rls).find("option[value='" + e + "']").attr("selected", "selected");
                    });
                }
                else {
                    $(rls).find("option[value='" + rule.filter.currentItem.value + "']").attr("selected", "selected");
                }
            }
            else {
                $(rls).removeAttr('multiple');
                rule.filter.multiple = false;
                $(rls).find("option[selected='selected']").removeAttr("selected");
                $(rls).find("option:first").attr("selected", "selected");
                $(rls).change();
            }
        });
    }, {
    ...
    QueryBuilder.regional['en'] = {
        ...
        "add_rule": "Add expression",
        "add_group": "Add group",
        "delete_rule": "Delete expression",
