...
{
    class GetTokenValueFunction : ParserFunction
    {
        protected override Parser.Result Evaluate(string data, ref int from)
        {
            ...
            {
                if (sourceValue.Token.Type == JTokenType.Array) { result.String = Convert.ToString(sourceValue.Token[0][propName]); }
                else
                {
                    if (propName.IndexOf(".") > -1)
                        result.String = Convert.ToString(JToken.FromObject(sourceValue.Token).SelectToken(propName));
                    else
                        result.String = Convert.ToString(sourceValue.Token[propName]);
               }
            }
            else
            {
                if (propName.IndexOf(".") > -1)
                {
                    result.String = Convert.ToString(JToken.Parse(sourceValue.String).SelectToken(propName));
                }
                else
