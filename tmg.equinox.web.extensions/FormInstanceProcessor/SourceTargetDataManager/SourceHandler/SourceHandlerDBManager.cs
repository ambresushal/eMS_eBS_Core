...
namespace tmg.equinox.web.sourcehandler
{
    public class SourceHandlerDBManager
    {
        ...
        public JObject GetMasterListData(int folderVersionId, List<string> sections, string ruleAlias, bool isMasterList,string mlDocumentFilter)
        {
            MasterListReader masterListReader = new MasterListReader(_tenantId, _masterListService);
            JObject masterSections = masterListReader.GetData(folderVersionId, sections, _formDataInstanceManager, _folderVersionServices, _formDesignServices, ruleAlias, isMasterList,mlDocumentFilter);
        ...
        public JObject GetCompleteFormData(int formInstanceId)
        {
            if(formInstanceId == 0)
            {
                return null;
            ...
            foreach(var section in designDetails.Sections)
            {
                var sectionStr = GetSectionData(section.FullName, formInstanceId);
                JToken sectionData = JToken.Parse(sectionStr);
                data.Add(section.FullName,sectionData.SelectToken(section.FullName));
            }
            return data;
        }
