...
namespace tmg.equinox.web.Framework.Caching
{
    public interface IDataCacheHandler
    {
        ...
        ServiceResult Save(IFolderVersionServices _folderVersionServices, int tenantId, int folderVersionId, int formInstanceId, string formInstanceData, string userName, int? userId, IReportingDataService _reportingDataService, int formDesignId, int formDesignVersionId);
    }
}
