...
namespace tmg.equinox.document.rulebuilder.ComplexRuleInterpreter.Extension
{
    public class TokenConstant
    {
        ...
        public const string COC_INTERVAL = "Interval";
        public const string SLID_COPAY = "Copay";
        public const string SLID_COPAYBEGIN = "CopayBeginDay";
        public const string SLID_COPAYEND = "CopayEndDay";
        public const string SLID_COIN = "Coinsurance";
        public const string SLID_COINBEGIN = "CoinsuranceBeginDay";
        public const string SLID_COINEND = "CoinsuranceEndDay";
    }
    public class FormatHelper
    {
        static List<string> _formats = new List<string>() { "$#", "#%", "$|%", "COB", "COC", "ANOCCHART", "SBVIEW" };
            ...
            if (HasFormat(format))
            {
                switch (format)
                {
                    ...
                    case "ANOCCHART":
                        ...
                        break;
                    case "SBVIEW":
                        result = SBDATAFormat(value.Token, param);
                        break;
                };
            }
        ...
        private static string SBDATAFormat(JToken value, string param)
        {
            string result = null;
            StringBuilder sb = new StringBuilder();
            if (value == null)
            ...
            else
            {
                ...
                else if (param == TokenConstant.COC_MAX)
                {
                    ...
                    sb.Append(IsEmpty(value[TokenConstant.COC_COINS_MAX]) ? "" : value[TokenConstant.COC_COINS_MAX].ToString() + "% of the cost");
                    if (string.IsNullOrWhiteSpace(sb.ToString()))
                    {
                        sb.Append("You pay nothing");
                ...
                else if (param == TokenConstant.COC_INTERVAL)
                {
                    sb.Append(IsEmpty(value[TokenConstant.SLID_COPAY]) ? "" : "$" + value[TokenConstant.SLID_COPAY].ToString() + " copay ");
                    if (IsEmpty(value[TokenConstant.SLID_COPAY]))
                    {
                        sb.Append(IsEmpty(value[TokenConstant.SLID_COIN]) ? "" : value[TokenConstant.SLID_COIN].ToString() + "% of the cost ");
                    ...
                    sb.Append(IsEmpty(checkDay(value)) ? "per stay" : checkDay(value)) ;
                ...
                else
                {
                    ...
                    sb.Append(IsEmpty(value[TokenConstant.COC_COINS]) ? "" : value[TokenConstant.COC_COINS].ToString() + "% of the cost");
                    if (string.IsNullOrWhiteSpace(sb.ToString()))
                    {
                        sb.Append("You pay nothing");
        ...
        private static string checkDay(JToken value)
        {
            string result = null;
            StringBuilder sb = new StringBuilder();
            if (!IsEmpty(value[TokenConstant.SLID_COPAY]))
            {
                sb.Append(IsEmpty(value[TokenConstant.SLID_COPAYBEGIN]) ? "" : " per day for Days " +value[TokenConstant.SLID_COPAYBEGIN].ToString() + " through ");
                if (!string.IsNullOrWhiteSpace(sb.ToString()))
                {
                    sb.Append(IsEmpty(value[TokenConstant.SLID_COPAYEND]) ? "" : value[TokenConstant.SLID_COPAYEND].ToString());

                }

            }
            else if (!IsEmpty(value[TokenConstant.SLID_COIN]))
            {
                sb.Append(IsEmpty(value[TokenConstant.SLID_COINBEGIN]) ? "" : " per day for Days " + value[TokenConstant.SLID_COINBEGIN].ToString() + " through ");
                if (!string.IsNullOrWhiteSpace(sb.ToString()))
                {
                    sb.Append(IsEmpty(value[TokenConstant.SLID_COINEND]) ? "" : value[TokenConstant.SLID_COINEND].ToString());
